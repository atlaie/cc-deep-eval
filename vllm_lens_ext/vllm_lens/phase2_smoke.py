...
Usage:
  export VLLM_BASE_URL="https://pre-pilot-phase2.debug.pour-demain.containers.tinfoil.dev/v1""
...
def check_routing(r, routing_layers, top_k=8):
    ...
    if float(entropy.min()) < 0:
        ...
        ok = False
    id_min, id_max = int(ids.min()), int(ids.max())
    if not (0 <= id_min and id_max < 256):
        print(f"  [FAIL] topk_ids out of [0, 256): min={id_min} max={id_max}")
        ok = False
    else:
        print(f"  [OK]   topk_ids range: min={id_min} max={id_max}")
    if ok:
...
def parse_args():
    ...
    p.add_argument("--base-url", default=os.environ.get("VLLM_BASE_URL", "https://pre-pilot-phase2.debug.pour-demain.containers.tinfoil.dev/v1"))
    p.add_argument("--api-key",  default=os.environ.get("VLLM_API_KEY", "tk_vawwLCV99Gpv0LIFvhec3sj0l5WoLKkoPmjJjEMzzowSwyYA"))
...
def main():
    ...
    # Cross-prompt variation: token-1+ routing must differ across prompts.
    # Token 0 (BOS) at L3 is deterministic — expected, not a bug.
    captured = [entry for entry in log if "routing" in entry]  # adapt to your log structure
    if len(captured) >= 2:
        # Deserialize first two toxic results and compare token-1 routing
        r0 = log[0].get("routing_raw")  # store deserialized routing in log entries
        r1 = log[2].get("routing_raw")  # pair 0 toxic, pair 1 toxic
        if r0 is not None and r1 is not None:
            ids0 = r0["topk_ids"]
            ids1 = r1["topk_ids"]
            if ids0.shape[1] > 1 and ids1.shape[1] > 1:
                if np.array_equal(ids0[:, 1, :], ids1[:, 1, :]):
                    print("\n[smoke] [FAIL] token-1 routing identical across prompts — degenerate capture")
                    all_ok = False
                else:
                    print("\n[smoke] [OK]   token-1 routing varies across prompts")
    if timings:
        print(f"\n[smoke] mean {np.mean(timings):.2f}s  min {min(timings):.2f}s  max {max(timings):.2f}s")
    else:
        print("\n[smoke] no successful calls")
