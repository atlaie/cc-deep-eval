...
# Each capture stream gets its own ContextVar so partial responses (e.g. only
# attention_stats requested) don't accidentally inherit a sibling's value.
_pending_activations: ContextVar[dict[str, Any] | None] = ContextVar(
    "_pending_activations", default=None
)
_pending_routing: ContextVar[dict[str, Any] | None] = ContextVar(
    "_pending_routing", default=None
)
_pending_attention_stats: ContextVar[dict[str, Any] | None] = ContextVar(
    "_pending_attention_stats", default=None
)
...
def _deserialize_routing(raw: dict[str, Any]) -> dict[str, Any]:
    """Deserialize tensors in a routing payload. layer_indices stays a list."""
    return {
        "layer_indices":   raw.get("layer_indices", []),
        "topk_ids":        deserialize_tensor(raw["topk_ids"]),
        "topk_weights":    deserialize_tensor(raw["topk_weights"]),
        "routing_entropy": deserialize_tensor(raw["routing_entropy"]),
    }


def _deserialize_attention_stats(raw: dict[str, Any]) -> dict[str, Any]:
    """Deserialize tensors in an attention_stats payload. layer_indices stays a list."""
    return {
        "layer_indices":    raw.get("layer_indices", []),
        "per_head_entropy": deserialize_tensor(raw["per_head_entropy"]),
        "per_head_max":     deserialize_tensor(raw["per_head_max"]),
        "top10pct_mass":    deserialize_tensor(raw["top10pct_mass"]),
    }
...
class VLLMLensAPI(VLLMAPI):
    ...
    activation capture, routing capture, attention-stats capture, and
    steering vector serialization.
    """
    ...
    def on_response(self, response: dict[str, Any]) -> None:  # pyright: ignore[reportGeneralTypeIssues]
        """Capture serialized activations, routing, attention_stats, and token IDs
        ...
        ``"routing"``, ``"attention_stats"`` (all injected by the vllm-lens
        server plugin) and ``"prompt_token_ids"`` / ``"token_ids"`` (returned
        by vLLM when ``return_token_ids`` is set).
        """
        _pending_activations.set(response.get("activations"))
        _pending_routing.set(response.get("routing"))
        _pending_attention_stats.set(response.get("attention_stats"))
    ...
    async def generate(  # pyright: ignore[reportGeneralTypeIssues]
        ...
        token_rout    = _pending_routing.set(None)
        token_attn    = _pending_attention_stats.set(None)
        ...
        try:
            ...
            raw_act = _pending_activations.get()
            if raw_act is not None:
                metadata["activations"] = {
                    name: deserialize_tensor(encoded) for name, encoded in raw_act.items()
                }

            raw_rout = _pending_routing.get()
            if raw_rout is not None:
                metadata["routing"] = _deserialize_routing(raw_rout)

            raw_attn = _pending_attention_stats.get()
            if raw_attn is not None:
                metadata["attention_stats"] = _deserialize_attention_stats(raw_attn)
            ...
            _pending_routing.reset(token_rout)
            _pending_attention_stats.reset(token_attn)
